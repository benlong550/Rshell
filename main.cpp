...
Base* grabTree(char *cstr) {
    ...
    if(checkingComment == NULL) {    //if there is a '#' in the first Token, disregard everything
        ...
        while(p!=0 && !commentDetected) {
            ...
            Command *testingCommand = new Command(p);
            char *q = p;
        ...
        if(connectorList.size() > 0) { //Only runs when there are 2 or more commands
            ...
            if(temp == AND_STRING) {
                ...
                input_redirection *n = new input_redirection(lhs, rhs);
                completedListToRun.push(n);
            }
            else if(temp == SINGLE_OUTPUT_RED) {
                output_redirection *n = new output_redirection(lhs, rhs);
                completedListToRun.push(n);
            }
            else if(temp == DOUBLE_OUTPUT_RED) {
                output_redirection2 *n = new output_redirection2(lhs, rhs);
                completedListToRun.push(n);
            }
            else if(temp == PIPE) {
            ...
            while(connectorList.size() != 0) {
                ...
                if(temp2 == AND_STRING) {
                    ...
                    input_redirection *n = new input_redirection(tempLHS, rhs);
                    completedListToRun.push(n);
                }
                else if(temp2 == SINGLE_OUTPUT_RED) {
                    output_redirection *n = new output_redirection(tempLHS, rhs);
                    completedListToRun.push(n);
                }
                else if(temp2 == DOUBLE_OUTPUT_RED) {
                    output_redirection2 *n = new output_redirection2(tempLHS, rhs);
                    completedListToRun.push(n);
                }
                else if(temp2 == PIPE) {
